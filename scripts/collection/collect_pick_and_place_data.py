...
def collect_episode(
    ...
    if not terminated:
        ...
        log.info(f"Episode {episode_idx:3d} | placed | final dist {final_dist:.4f} m")
...
def main() -> None:
    cfg = load_config()
    col = cfg["pick_and_place_collection"]
    ...
    env = PickAndPlaceEnvironment(**cfg["pick_and_place_env"])
    expert = PickAndPlaceExpert(env, **cfg["pick_and_place_expert"])
    dataset = DatasetManager(
